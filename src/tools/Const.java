...
public class Const {
	...
	public static final String SQL_TLLR_OPER = "SELECT * FROM ADM_OPERATION WHERE SYS_NO = 1360 AND UPPER(LOOKUP) LIKE UPPER('%Tllr{module}Tran%')";
	public static final String SQL_TLLR_PRIV = "SELECT P.* FROM ADM_PRIVILEGE P INNER JOIN (SELECT PRIV_CODE FROM ADM_OPER_PRIV OP LEFT JOIN "
			+ "ADM_OPERATION O ON OP.OPER_CODE = O.OPER_CODE WHERE O.SYS_NO = 1360 AND UPPER (O.LOOKUP) LIKE UPPER ('%Tllr{module}Tran%') "
			+ "GROUP BY PRIV_CODE) OP ON OP.PRIV_CODE = P.PRIV_CODE";
	public static final String SQL_TLLR_OPER_PRIV = "SELECT OP.* FROM ADM_OPER_PRIV OP LEFT JOIN ADM_OPERATION O ON OP.OPER_CODE = O.OPER_CODE "
			+ "WHERE O.SYS_NO = 1360 AND UPPER (O.LOOKUP) LIKE UPPER ('%Tllr{module}Tran%')";
	
	public static final String SQL_VIEW = "SELECT VIEW_NAME, TEXT FROM ALL_VIEWS WHERE (UPPER(VIEW_NAME) LIKE UPPER('VW_{module}%') "
	...
	public static final String SQL_MERGE_GEN_SYSTEM = "MERGE INTO GEN_SYSTEM A USING (SELECT {param1} as SYS_NO, '{param2}' "
			+ " as NAME, '{param3}' as NAME2, {param4} as STATUS, 1 as CREATED_BY, SYSDATE as CREATED_DATETIME, 1 as MODIFIED_BY, "
			+ " SYSDATE as MODIFIED_DATETIME, {param5} as ORDER_NO FROM DUAL) B ON (A.SYS_NO = B.SYS_NO) WHEN NOT MATCHED THEN "
	...
	public static final String SQL_MERGE_TXN_CODE = "SELECT 'MERGE INTO BCOM_TXN_CODE A USING (SELECT ' || SYS_NO || ' AS SYS_NO, ' || '''' || PROD_TYPE || ''' "
			+ "AS PROD_TYPE, ' || '''' || TXN_CODE || ''' AS TXN_CODE, ' || '''' || NAME || ''' AS NAME, ' || '''' || NAME2 || ''' AS NAME2, ' || '''' || TXN_TYPE "
			+ "|| ''' AS TXN_TYPE, ' || '''' || TXN_GROUP || ''' AS TXN_GROUP, ' || IN_CUST_STMT || ' AS IN_CUST_STMT, ' || IN_BANK_STMT || ' AS IN_BANK_STMT ' || "
			+ "'FROM DUAL) B ON (A.PROD_TYPE = B.PROD_TYPE and A.TXN_CODE = B.TXN_CODE) WHEN NOT MATCHED THEN INSERT (SYS_NO, PROD_TYPE, TXN_CODE, NAME, NAME2, "
			+ "TXN_TYPE, TXN_GROUP, IN_CUST_STMT, IN_BANK_STMT) VALUES (B.SYS_NO, B.PROD_TYPE, B.TXN_CODE, B.NAME, B.NAME2, B.TXN_TYPE, B.TXN_GROUP, B.IN_CUST_STMT, "
			+ "B.IN_BANK_STMT)~~' MRG_QUERY FROM BCOM_TXN_CODE WHERE SYS_NO = {system}";
	...
	public static final String SQL_MERGE_GL_CFG = "SELECT 'MERGE INTO GLIP_GL_CFG A USING (SELECT ' || 'COMPANY_CODE, ' || '''' || A.PROD_TYPE || ''' AS "
			+ "PROD_TYPE, ' || '''' || A.BAL_TYPE_CODE || ''' AS BAL_TYPE_CODE, ' || A.BY_PROD || ' AS BY_PROD, ' || A.BY_CLASS || ' AS BY_CLASS ' "
			+ "|| 'FROM GEN_COMPANY) B ON (A.COMPANY_CODE = B.COMPANY_CODE and A.PROD_TYPE = B.PROD_TYPE and A.BAL_TYPE_CODE = B.BAL_TYPE_CODE) "
			+ "WHEN NOT MATCHED THEN INSERT (COMPANY_CODE, PROD_TYPE, BAL_TYPE_CODE, BY_PROD, BY_CLASS) VALUES (B.COMPANY_CODE, B.PROD_TYPE, "
			+ "B.BAL_TYPE_CODE, B.BY_PROD, B.BY_CLASS)~~' MRG_QUERY FROM GLIP_GL_CFG A LEFT JOIN BCOM_PROD_TYPE P ON A.PROD_TYPE = P.PROD_TYPE "
			+ "WHERE A.COMPANY_CODE = '10' AND P.SYS_NO = {system}";
	public static final String SQL_MERGE_GL_CONT_CFG = "SELECT 'MERGE INTO GLIP_CONT_ENTRY_CFG A USING (SELECT ' || '''' || A.PROD_TYPE || ''' AS "
			+ "PROD_TYPE, ' || '''' || A.BAL_TYPE_CODE || ''' AS BAL_TYPE_CODE, ' || '''' || A.TXN_GROUP || ''' AS TXN_GROUP, ' || '''' || "
			+ "A.CONT_BAL_TYPE_CODE || ''' AS CONT_BAL_TYPE_CODE, ' || A.OFFBAL || ' AS OFFBAL ' || 'FROM DUAL) B ON (A.PROD_TYPE = B.PROD_TYPE "
			+ "and A.BAL_TYPE_CODE = B.BAL_TYPE_CODE and A.TXN_GROUP = B.TXN_GROUP and A.OFFBAL = B.OFFBAL) WHEN NOT MATCHED THEN "
			+ "INSERT (PROD_TYPE, BAL_TYPE_CODE, TXN_GROUP, CONT_BAL_TYPE_CODE, OFFBAL) VALUES (B.PROD_TYPE, B.BAL_TYPE_CODE, B.TXN_GROUP, "
			+ "B.CONT_BAL_TYPE_CODE, B.OFFBAL)~~' MRG_QUERY FROM GLIP_CONT_ENTRY_CFG A LEFT JOIN BCOM_PROD_TYPE P ON A.PROD_TYPE = P.PROD_TYPE "
			+ "WHERE P.SYS_NO = {system}";
	public static final String SQL_MERGE_GL_TXN_CFG = "SELECT 'MERGE INTO GLIP_TXN_CONFIG A USING (SELECT ' || SYS_NO || ' AS SYS_NO, ' || '''' "
			+ "|| TXN_CODE || ''' AS TXN_CODE, ' || TXN_ITEM_NO || ' AS TXN_ITEM_NO, ' || '''' || DR_OR_CR || ''' AS DR_OR_CR, ' || '''' "
			+ "|| BAL_TYPE_CODE || ''' AS BAL_TYPE_CODE, ' || PROD_CODE_PARAM || ' AS PROD_CODE_PARAM, ' || SEG_CODE_PARAM || ' AS SEG_CODE_PARAM, ' "
			+ "|| CLASS_NO_PARAM || ' AS CLASS_NO_PARAM, ' || BRCH_CODE_PARAM || ' AS BRCH_CODE_PARAM ' || 'FROM DUAL) B ON (A.TXN_ITEM_NO ="
			+ " B.TXN_ITEM_NO and A.TXN_CODE = B.TXN_CODE and A.SYS_NO = B.SYS_NO) WHEN NOT MATCHED THEN INSERT (SYS_NO, TXN_CODE, TXN_ITEM_NO, "
			+ "DR_OR_CR, BAL_TYPE_CODE, PROD_CODE_PARAM, SEG_CODE_PARAM, CLASS_NO_PARAM, BRCH_CODE_PARAM) VALUES (B.SYS_NO, B.TXN_CODE, B.TXN_ITEM_NO, "
			+ "B.DR_OR_CR, B.BAL_TYPE_CODE, B.PROD_CODE_PARAM, B.SEG_CODE_PARAM, B.CLASS_NO_PARAM, B.BRCH_CODE_PARAM)~~' MRG_QUERY FROM "
			+ "GLIP_TXN_CONFIG WHERE SYS_NO = {system}";
	public static final String SQL_MERGE_LM_OPER = "SELECT 'MERGE INTO LM_OPER A USING (SELECT ' || '''' || L.OPER_CODE || ''' AS OPER_CODE, ' || '''' "
			+ "|| L.NAME || ''' AS NAME, ' || '''' || L.NAME2 || ''' AS NAME2, ' || L.STATUS || ' AS STATUS, ' || L.SYS_NO || ' AS SYS_NO ' || 'FROM DUAL) "
			+ "B ON (A.OPER_CODE = B.OPER_CODE) WHEN NOT MATCHED THEN INSERT (OPER_CODE, NAME, NAME2, STATUS, SYS_NO) VALUES (B.OPER_CODE, B.NAME, B.NAME2, "
			+ "B.STATUS, B.SYS_NO)~~' MRG_QUERY FROM LM_OPER L INNER JOIN ADM_OPERATION O ON O.OPER_CODE = L.OPER_CODE WHERE O.SYS_NO = 1360 AND "
			+ "UPPER (O.LOOKUP) LIKE UPPER ('%Tllr{module}Tran%')";
